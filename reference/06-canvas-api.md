...
Canvas throttles bursts. The code keeps Canvas calls sequential (tools execute one at a time; `ensureCollections` runs in series; the courses sync fetches each course's tabs in series). There is no retry/backoff.
