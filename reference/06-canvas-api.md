...
- The browser can revoke optional permissions (Firefox lets the user withdraw any host permission, the known instances included, from the add-on's Permissions tab), so `hasOriginPermission` runs on every start and the Connect screen returns when it is missing. A known instance's permission cannot be released by the extension, so "Switch Canvas" only stops auto-connecting for the session and shows the Connect screen.
