...
|---|---|---|
| `Message` (`components/ChatUI`) | Display bubbles: `role`, `content`, `timestamp`. Only user and assistant text. | Yes, `Chat.messages` |
...
user text
  ...
  → loop (max MAX_TOOL_ROUNDS = 12):
      callLLM(buildApiHistory(...), settings)      (tools included)
      parse function calls (either provider format)
      if text: append assistant display message
      append assistant ConversationMessage (with toolCalls + signatures)
      if no function calls: break
      execute each tool sequentially → append a toolResults turn
  → cap persisted tool results, ensureContextWithinThreshold(), save
  → on error: append an "Error: …" bubble; drop a dangling tool-call turn so the next request is well-formed
```
