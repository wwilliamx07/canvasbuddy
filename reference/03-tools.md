...
| `search_documents(query, course_id?, document_type?, document_id?, limit?)` | best excerpts across indexed documents and inbox messages, with page/slide for citation | (`inbox` when `document_type="conversation"`) | With `document_type + document_id` the document is indexed if needed (`indexDocumentJustInTime`) and the search is restricted to it — the common "what does lecture 4 say about X" path is `list_content(items)` → `search_documents(document=…)`. Without them, only previously indexed documents are searched. |
...
| `get_inbox(scope?, search?, limit?, refresh?)` | conversations newest first, snippet of last message; `search` also matches stored message bodies (FTS) and returns `matching_message` | `inbox` | Threads are fetched and their messages stored/embedded during the inbox sync, so `read_document(conversation)` and `search_documents` work on them without another tool. |
