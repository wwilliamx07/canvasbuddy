...
- **Schema changes go in the `CREATE TABLE` in `schema.ts`; there are no migrations yet.** `SCHEMA_SQL` runs on every start and is all `IF NOT EXISTS`, so it completes a new database and is a no-op on an existing one. While there are no users, a changed schema means forgetting the memory and starting fresh (cheap, given the freshness engine and lazy indexing) — don't write `ALTER TABLE` / backfill statements for that. Once there are installs to keep, migrations are a legitimate addition; the decision is about whether anyone's data is worth carrying, not about the schema file being declarative.
...
- **Everything under `src/ui` reads `AppModel` and nothing else.** `App.tsx` builds the model; components take it (or a slice) as props. The two hooks (`useMemoryExplorer`, `useConnectFlow`) are the only files there that import from `db/*` or `canvas/*`, and they exist to fill the model. A component that needs data it does not have gets it through the model, not through an import.
